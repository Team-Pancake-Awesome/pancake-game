...
public class JoystickSpatulaInput : MonoBehaviour, ISpatulaInput, ISpatulaInputBackgroundActivity
{
    ...
    [Tooltip("Fallback potentiometer value if the joystick does not expose a usable trigger axis.")]
    [Range(0f, 1f)]
    public float fallbackPotValue = 0f;

    [Header("Action Button Mapping")]
    [Tooltip("Specific joystick button name to use for action. Leave blank to use trigger > 0.5 only.")]
